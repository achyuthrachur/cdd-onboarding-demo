    ...
    return (
      <Card className="h-full flex items-center justify-center">
        <div className="text-center">
          <User className="h-16 w-16 mx-auto mb-4 text-tint-700 dark:text-white/80 opacity-30" />
      ...
      <CardHeader className="flex-shrink-0 border-b">
        ...
        <div className="grid grid-cols-3 lg:grid-cols-6 gap-3">
          ...
          <div className="p-3 rounded-lg text-center flex flex-col items-center justify-center min-h-[80px] bg-tint-100 dark:bg-white/10">
            <div className="text-lg font-bold text-tint-700 dark:text-white/80">{summary.naCount}</div>
            <div className="text-xs text-tint-700 dark:text-white/80">N/A</div>
          </div>
        </div>
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between text-sm mb-1">
            <span className="text-tint-700 dark:text-white/80">Completion Progress</span>
            <span className="font-medium text-tint-900 dark:text-white">{summary.completionPercentage}%</span>
          </div>
          <Progress value={summary.completionPercentage} className="h-2 [&>div]:h-2" />
          <div className="text-xs text-tint-700 dark:text-white/80 mt-1">
        ...
        <div className="flex gap-3 mt-4">
          <div className="flex items-center gap-2">
            <Filter className="h-4 w-4 text-tint-700 dark:text-white/80" />
          ...
          <TableBody>
            {filteredRows.map((row) => (
              <TableRow key={row.id}>
                ...
                <TableCell className="px-4 py-2 text-xs text-tint-700 dark:text-white/80 truncate max-w-[200px]" title={row.observation}>
                  {row.observation || "-"}
                </TableCell>
              </TableRow>
            ))}
            {filteredRows.length === 0 && (
              <TableRow>
                <TableCell colSpan={7} className="text-center py-8 text-tint-700 dark:text-white/80">
