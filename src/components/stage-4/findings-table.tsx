      ...
      <Card>
        ...
        <CardContent>
          {findingsByAttribute.length === 0 ? (
            <motion.div
              className="text-center py-8 text-gray-600 dark:text-white/80"
            ...
            >
              {findingsByAttribute.map((attr, index) => (
                ...
                >
                  <div
                    className="flex items-center justify-between p-4 cursor-pointer hover:bg-gray-100 dark:hover:bg-white/5"
                      ...
                      attr.observations.length > 0 && (
                        ...
                        >
                          <div className="px-4 pb-4 pt-0">
                            <div className="pl-8 border-l-2 border-gray-300 dark:border-white/20 ml-2">
                              <p className="text-sm font-medium mb-2 text-gray-700 dark:text-white/80">
                                Observations:
                              </p>
                              <ul className="space-y-1">
                                {attr.observations.map((obs, idx) => (
                                  <motion.li
                                    key={idx}
                                    className="text-sm text-gray-600 dark:text-white/80 flex items-start gap-2"
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredExceptions.map((exception, index) => (
                      <motion.tr
                        key={exception.id}
                        className="border-b transition-colors hover:bg-gray-100 dark:hover:bg-white/5"
                        initial={shouldReduceMotion ? undefined : { opacity: 0, x: -10 }}
                        animate={{ opacity: 1, x: 0 }}
        ...
        <DialogContent className="max-w-lg">
          ...
          {selectedFinding && (
            <div className="space-y-4">
              ...
              <div>
                <p className="text-sm font-medium text-gray-600 dark:text-white/80 mb-1">
                  Observation
                </p>
              ...
              {selectedFinding.auditorNotes && (
                <div>
                  ...
                  <p className="p-3 bg-gray-100 dark:bg-white/5 rounded-lg text-sm text-gray-700 dark:text-white/80">
                    {selectedFinding.auditorNotes}
                  </p>
                </div>
              )}
