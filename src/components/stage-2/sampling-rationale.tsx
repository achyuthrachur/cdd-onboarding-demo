      ...
      <CardContent>
        ...
        {rationale ? (
          <div className="space-y-4">
            <div className="prose dark:prose-invert max-w-none rounded-lg bg-tint-50 dark:bg-white/5 p-6 border border-tint-200 dark:border-white/10">
              <ReactMarkdown
                components={{
                  h2: ({ children }) => (
                    <h2 className="text-xl font-bold text-tint-900 dark:text-white mt-6 mb-4 first:mt-0 border-b border-tint-200 dark:border-white/20 pb-2">
                      {children}
                    </h2>
                  ),
                  h3: ({ children }) => (
                    <h3 className="text-lg font-semibold text-tint-900 dark:text-white mt-4 mb-2">
                      {children}
                    </h3>
                  ),
                  p: ({ children }) => (
                  ...
                  ul: ({ children }) => (
                    <ul className="text-tint-700 dark:text-white/80 list-disc list-inside mb-3 space-y-1">
                      {children}
                    </ul>
                  ),
                  li: ({ children }) => (
                  ...
                  table: ({ children }) => (
                    <div className="overflow-x-auto my-4">
                      <table className="w-full border-collapse border border-tint-200 dark:border-white/20 text-sm">
                        {children}
                      </table>
                    </div>
                  ),
                  thead: ({ children }) => (
                    <thead className="bg-tint-100 dark:bg-white/10">{children}</thead>
                  ),
                  th: ({ children }) => (
                    <th className="border border-tint-200 dark:border-white/20 px-4 py-2 text-left text-tint-900 dark:text-white font-semibold">
                      {children}
                    </th>
                  ),
                  td: ({ children }) => (
                    <td className="border border-tint-200 dark:border-white/20 px-4 py-2 text-tint-700 dark:text-white/80">
                      {children}
                    </td>
                  ),
                  hr: () => <hr className="border-tint-200 dark:border-white/20 my-6" />,
                  em: ({ children }) => (
            ...
            <div className="flex justify-end">
              ...
              >
                {copied ? (
                  <>
                    <Check className="h-4 w-4 mr-2 text-crowe-teal" />
                    Copied!
                  </>
                ) : (
                  <>
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            <Sparkles className="h-12 w-12 text-tint-500 dark:text-white/50 mb-4" />
            <p className="text-tint-700 dark:text-white/80 mb-2">
              Click &quot;Generate Rationale&quot; to create audit documentation
            </p>
            <p className="text-sm text-tint-500 dark:text-white/50">
